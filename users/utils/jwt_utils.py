...
def criar_token(user):
    ...
    refresh_token = jwt.encode(
        refresh_payload,
        settings.SECRET_KEY
        + user.password,  # usa a senha, senha alterada = token invalidado
        algorithm="HS256",
    )
...
def verificar_token_cookies(request):
    ...
    if not access_token:
        return None, JsonResponse(
            {"error": "Token de acesso não encontrado."}, status=401
        )
    ...
    try:
        ...
        return None, JsonResponse(
            {"error": "Usuário associado ao token não encontrado."}, status=401
        )
        ...
        if not refresh_token:
            return None, JsonResponse(
                {"error": "Sessão expirada. Faça login novamente."}, status=401
            )
        ...
        try:
            ...
            if unverified_payload.get("token_type") != "refresh":
                return None, JsonResponse(
                    {"error": "Tipo de token inválido"}, status=401
                )

            # Busca o usuário no banco de dados.
            user = Users.objects.get(id=unverified_payload["id"])

            # AGORA, constrói a chave secreta correta e verifica a assinatura do refresh token.
            jwt.decode(
                refresh_token, settings.SECRET_KEY + user.password, algorithms=["HS256"]
            )
        ...
        except (jwt.DecodeError, jwt.InvalidTokenError, Users.DoesNotExist):
            return None, JsonResponse(
                {"error": "Não foi possível validar a sessão."}, status=401
            )
